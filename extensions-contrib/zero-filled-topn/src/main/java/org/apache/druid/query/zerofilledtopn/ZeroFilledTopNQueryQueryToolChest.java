...
public class ZeroFilledTopNQueryQueryToolChest extends QueryToolChest<Result<TopNResultValue>, ZeroFilledTopNQuery>
{

  // use the MAX_VALUE to avoid potential collision
  private static final byte ZEROFILLED_TOPN_QUERY = Byte.MAX_VALUE;
  ...
  private final ZeroFilledTopNQueryMetricsFactory queryMetricsFactory;

  @VisibleForTesting
  public ZeroFilledTopNQueryQueryToolChest(
      TopNQueryConfig config
  )
  {
    this(config, DefaultZeroFilledTopNQueryMetricsFactory.instance());
  }

  @Inject
  public ZeroFilledTopNQueryQueryToolChest(
      TopNQueryConfig config,
      ZeroFilledTopNQueryMetricsFactory queryMetricsFactory
  )
  {
    this.config = config;
  ...
  {
    ZeroFilledTopNQuery query = (ZeroFilledTopNQuery) input;
  ...
  public Function<Result<TopNResultValue>, Result<TopNResultValue>> makePreComputeManipulatorFn(
      final ZeroFilledTopNQuery query,
  ...
  @Override
  public CacheStrategy<Result<TopNResultValue>, Object, ZeroFilledTopNQuery> getCacheStrategy(final ZeroFilledTopNQuery query)
  {
    return new CacheStrategy<Result<TopNResultValue>, Object, ZeroFilledTopNQuery>()
    {
      ...
      @Override
      public boolean isCacheable(ZeroFilledTopNQuery query, boolean willMergeRunners)
      {
        return true;
      }

      @Override
      public byte[] computeCacheKey(ZeroFilledTopNQuery query)
      {
        final CacheKeyBuilder builder = new CacheKeyBuilder(ZEROFILLED_TOPN_QUERY)
            ...
            .appendStrings(query.getDimValues());
      ...
      @Override
      public byte[] computeResultLevelCacheKey(ZeroFilledTopNQuery query)
      {
        final CacheKeyBuilder builder = new CacheKeyBuilder(ZEROFILLED_TOPN_QUERY)
            .appendCacheable(query.getDimensionSpec())
            .appendCacheable(query.getTopNMetricSpec())
  ...
  public QueryRunner<Result<TopNResultValue>> preMergeQueryDecoration(final QueryRunner<Result<TopNResultValue>> runner)
  {
    ...
    return (queryPlus, responseContext) -> {
      ZeroFilledTopNQuery zeroFilledTopNQuery = (ZeroFilledTopNQuery) queryPlus.getQuery();
      if (TopNQueryEngine.canApplyExtractionInPost(zeroFilledTopNQuery.toTopNQuery())) {
        final DimensionSpec dimensionSpec = zeroFilledTopNQuery.getDimensionSpec();
        QueryPlus<Result<TopNResultValue>> delegateQueryPlus = queryPlus.withQuery(
            zeroFilledTopNQuery.withDimensionSpec(
        ...
        return runner.run(queryPlus.withQuery(zeroFilledTopNQuery), responseContext);
      }
    };
  }
  ...
  {
    final ZeroFilledTopNQueryQueryToolChest.ThresholdAdjustingQueryRunner thresholdRunner = new ZeroFilledTopNQueryQueryToolChest.ThresholdAdjustingQueryRunner(
        runner,
        config
    );
    return (queryPlus, responseContext) -> {
      // thresholdRunner.run throws ISE if query is not ZeroFilledTopNQuery
      final Sequence<Result<TopNResultValue>> resultSequence = thresholdRunner.run(queryPlus, responseContext);
      final ZeroFilledTopNQuery zeroFilledTopNQuery = (ZeroFilledTopNQuery) queryPlus.getQuery();
      if (!TopNQueryEngine.canApplyExtractionInPost(zeroFilledTopNQuery.toTopNQuery())) {
        ...
        return Sequences.map(
            resultSequence,
            result -> {
              TopNResultValue resultValue = result.getValue();
              ...
              return new Result<>(
                  ...
                  new TopNResultValue(
                      Lists.transform(
                          ...
                          extractor -> {
                            String dimOutputName = zeroFilledTopNQuery.getDimensionSpec().getOutputName();
                            Object dimValue = extractor.getDimensionValue(dimOutputName);
                            Map<String, Object> map = extractor.getBaseObject();
                            map.put(
                                dimOutputName,
                                zeroFilledTopNQuery.getDimensionSpec().getExtractionFn().apply(dimValue)
                            );
                            return extractor;
                          }
                      )
  ...
  @Override
  public RowSignature resultArraySignature(ZeroFilledTopNQuery query)
  {
  ...
  @Override
  public Sequence<Object[]> resultsAsArrays(ZeroFilledTopNQuery query, Sequence<Result<TopNResultValue>> resultSequence)
  {
  ...
  static class ThresholdAdjustingQueryRunner implements QueryRunner<Result<TopNResultValue>>
  {
    ...
    )
    {
      ...
      if (!(input instanceof ZeroFilledTopNQuery)) {
        throw new ISE("Can only handle [%s], got [%s]", ZeroFilledTopNQuery.class, input.getClass());
      }

      final ZeroFilledTopNQuery query = (ZeroFilledTopNQuery) input;
      ...
      return Sequences.map(
          ...
          result -> {
            if (isBySegment) {
              BySegmentResultValue<Result<TopNResultValue>> value = (BySegmentResultValue<Result<TopNResultValue>>) result
                  .getValue();
              ...
              return new Result<>(
                  ...
                  new BySegmentTopNResultValue(
                      Lists.transform(
                          value.getResults(),
                          topNResultValue -> new Result<>(
                              topNResultValue.getTimestamp(),
                              new TopNResultValue(
                                  Lists.<Object>newArrayList(
                                      Iterables.limit(
                                          topNResultValue.getValue(),
                                          query.getThreshold()
                                      )
                                  )
                              )
